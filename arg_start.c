...
bool    next_arg_valid(char *arg, int type)
{
    ...
    else if (type == INT)
    {
        if(valid_int(arg))
        {
            ...
            sprintf(verif, "%d", atoi(arg));
            if(strcmp(verif, arg) == 0)
            {
                free(verif);
                return true;
            }
...
void    multiple_definition(arg_opt *tab, int i, int *valid)
{
    if (tab->arg[i][1] == '-')
    {
        ...
        {
            if (strcmp(tab->arg[i], tab->options[j].fullname_opt) == 0)
                break ;
        }
        if(j == tab->nb_options)
            (valid)++;
    ...
    else
    {
        ...
        for(j = 0; j < tab->nb_options; j++)
        {
            if (strcmp(tab->arg[i], tab->options[j].opt) == 0)
                break ;
        }
        if(j == tab->nb_options)
        {
...
char    **arg_without_opt(char **arg, arg_opt *tab)
{
    ...
    for(int i = 0; arg[i]; i++)
    {
        if (arg[i][0] != '-')
        {
            ...
            {
                rst = realloc(rst, (sizeof(char*) * size));
                rst[j++] = arg[i];
                size++;
            }
